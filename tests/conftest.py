...
from lightspeed_suite.config import QueryConfig, SuiteConfig, load_config
...
@pytest.fixture(scope="session")
def query_config(config: SuiteConfig) -> QueryConfig:
    """All API requests use OpenAI; vLLM is reserved for validation."""

    return config.query_config
...
def case_context(
    ...
    @contextmanager
    def _ctx(provider: str | None = "openai") -> Iterator[CaseRecord]:
        ...
        try:
            ...
            result_logger.write_case(
                ...
                notes=record.notes,
                query_provider=provider or "openai",

...
def _request_snapshot(
    user_id: str, provider: QueryConfig, query: str, extra: dict[str, Any] | None = None
) -> dict[str, Any]:
...
def test_streaming_query_returns_ok(
    ...
    with case_context(provider=query_config.provider) as record:
        query = config.standard_query
        record.set_request(_request_snapshot(user_id, query_config, query))
...
def test_streaming_query_with_rag_has_referenced_documents(
    ...
    with case_context(provider=query_config.provider) as record:
        record.set_request(_request_snapshot(user_id, query_config, config.rag_query))
        response = client.streaming_query(
            user_id=user_id,
            provider=query_config.provider,
            model=query_config.model,
...
def test_referenced_documents_are_present_in_conversation(
    ...
    with case_context(provider=query_config.provider) as record:
        ...
        record.set_request(
            _request_snapshot(user_id, query_config, config.rag_query, {"conversation_id": conversation_id})
        )
        record.set_response(
            {
...
def test_conversation_created_and_accessible_by_user(
    ...
    with case_context(provider=query_config.provider) as record:
        ...
        convo_response = client.get_conversation(conversation_id, user_id)
        convo_payload = _response_json(convo_response)
        record.set_request(_request_snapshot(user_id, query_config, config.standard_query))
        record.set_response(
            {
                "stream": _stream_response_snapshot(response),
                "conversation_status_code": convo_response.status_code,
...
def test_validation_allows_developer_query(
    client: LightspeedClient,
    config: SuiteConfig,
    query_config: QueryConfig,
    user_id: str,
    ...
    """Verify vLLM validation allows the normal OpenAI query path."""
    ...
    with case_context(provider=query_config.provider) as record:
        ...
        record.set_response(_stream_response_snapshot(response))
        assert response.status_code == 200
        assert response.events, "Expected allowed developer query to stream events"
...
def test_safety_guard_catches_problematic_queries(
    ...
    with case_context(provider=query_config.provider) as record:
        ...
        record.set_response(_stream_response_snapshot(response))
        assert response.status_code == 200
        expected_rejection = extract_validation_rejection(config.llama_stack_config_path)
        actual_text = _assemble_stream_text(response.events)
        assert normalize_response_text(expected_rejection) == normalize_response_text(actual_text), (
            "Expected problematic query to return the configured validation rejection"
        )
